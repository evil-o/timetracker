...
export class ActivitiesComponent {
  ...
  constructor(public store: Store<ApplicationState>, private modalService: BsModalService) {
    this.activities$ = store.select(fromStore.activityTypes).pipe(map(types => types.activities));
    ...
    this.mergeSource$.pipe(withLatestFrom(this.activities$)).subscribe(([source, activities]) => {
      this.activitiesWithoutSource = activities.filter(a => source === undefined || a.id !== source.id);
    });

    this.confirmMerge$.pipe(withLatestFrom(this.activities$)).subscribe(([merge, activities]) => {
