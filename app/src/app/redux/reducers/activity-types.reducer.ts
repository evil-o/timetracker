...
export const activityTypesReducer = createReducer(
  ...
  produceOn(activityTypeActions.setNonWorking, (draft, { id, isNonWorking }) => {
  ...
  produceOn(activityTypeActions.setColorId, (draft, { activityTypeId, colorId }) => {
  ...
  produceOn(activityTypeActions.import, (draft, { data }) => {
    if (data.activities) {
      for (const activity of data.activities) {
        const match = draft.activities.find((a) => a.id === activity.id);
        if (match) {
