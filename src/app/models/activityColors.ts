...
const baseColorsHsv = baseColorsRgb.map((c) => ({ ...c, color: convert.rgb.hsv(c.color) }));

// create variants for each color
const variantsHSV = baseColorsHsv.map((c) => {
  // initialize with the base variant
  const colorVariants: IColor[] = [c];
  // decrease value by a bit
  let index = 2;
  for (const decrement of [10, 20, 30, 35]) {
    colorVariants.push({ name: `${c.name}-${index}`, color: [c.color[0], c.color[1], c.color[2] - decrement] });
    index += 1;
  }
  return colorVariants;
}).reduce((prev, cur) => prev.concat(cur), []);

const variantsRGB = variantsHSV.map((c) => ({ ...c, color: convert.hsv.rgb(c.color) }));
